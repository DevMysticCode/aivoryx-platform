...
- Redis is used **only** for BullMQ queues (and, later, optional non-authoritative
  caches). It is **not** the authoritative store for sessions or any other state
  — application correctness must not depend on Redis being available (ADR 0010).
